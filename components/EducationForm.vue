...
<template>
  <div>
    ...
    <form v-if="formStatus !== 'finished'" @submit="submit">
      ...
      <FormElement :label="$t('forms.label.school')" required="true">
        <FormInput v-model="form.school" :placeholder="$t('forms.placeholder.school')" />
        <FormValidation name="school" :errors="validationErrors" />
      </FormElement>
      <FormElement :label="$t('forms.label.message')" required="true">
