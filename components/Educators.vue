          ...
          <div v-show="index === active" class="absolute w-full text-center">
            ...
            <div class="bg-white-gradient-vertical">
              <div>
                ...
                <div class="text-gray-500 italic" v-text="testimonial[`pronouns_${$i18n.locale}`]" />
              </div>
              <div class="flex px-6 pb-6 leading-snug">
                <div class="absolute text-mega text-brand-100 -ml-6 -mt-20 pt-2 z-0">“</div>
                <div class="text-xl z-20 pt-2" v-text="testimonial[`quote`]" />
              </div>
            </div>
          </div>
        </transition>
            ...
            <div class="flex-1 bg-white-gradient mr-12">
              <div class="flex-1 flex flex-col justify-between relative p-3 my-6">
                ...
                <div>
                  ...
                  <span class="text-gray-500 italic" v-text="testimonial[`pronouns_${$i18n.locale}`]" />
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  async fetch() {
    this.testimonials = await this.$content('educators').fetch()
  },
  mounted() {
    this.startSliding()
  },
