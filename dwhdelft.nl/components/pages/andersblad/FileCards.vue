...
const filesForDisplay = computed(() => files.value.map((file) => ({
  ...file,
...
<template>
  <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-2 md:gap-4">
    <ElementsActionCard v-for="file in filesForDisplay" :key="file.id" :title="file.name" :content="file.publishDate" titleClass="text-xl" contentClass="px-4 pt-6 pb-3">
      <template #header>
